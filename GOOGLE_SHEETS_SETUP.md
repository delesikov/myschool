...
|------------|---------------|------|-------|------|------|-----------|
| a1b2c3d4 | 21.10.2025 10:25:00 | 21.10.2025 | 10:30:00 | Единицы измерения веса | Ученик | Привет! |
| a1b2c3d4 | 21.10.2025 10:25:00 | 21.10.2025 | 10:30:05 | Единицы измерения веса | Тьютор | Привет! Давай разберем... |
| x9y8z7w6 | 21.10.2025 11:15:00 | 21.10.2025 | 11:20:00 | Пропорции | Ученик | Помоги с пропорциями |

**Session ID** - уникальный идентификатор диалога. Все сообщения из одной сессии имеют одинаковый ID, что позволяет группировать их вместе.

Вы сможете:
- **Группировать сообщения** по Session ID - видеть весь диалог целиком
- **Отслеживать прогресс** учеников по разным темам
- **Анализировать частые ошибки** в конкретных диалогах
- **Фильтровать** по темам, датам и сессиям
- **Делать статистику** - сколько диалогов, по каким темам, длительность обучения
...
3. **Группировка по Session ID**: Используйте фильтры или сортировку по Session ID, чтобы видеть все сообщения из одного диалога вместе
4. **Анализ**: Используйте Google Sheets формулы для анализа:
   - `=COUNTUNIQUE(A:A)` - количество уникальных сессий
   - `=COUNTIF(E:E, "Единицы измерения веса")` - сколько сообщений по конкретной теме
5. **Фильтры**: Создайте фильтры по темам, датам и Session ID для удобного поиска
6. **Pivot Tables**: Создайте сводные таблицы для анализа активности по темам и датам
