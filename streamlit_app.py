...
- **Возраст** (или класс)
- **Предмет и тему**, которую нужно изучить (или конкретное задание/пример)
- **Цель** — понять тему, решить задачу, подготовиться к контрольной или что-то другое?
...
def parse_quiz_options(text: str):
    """Парсит варианты ответов из текста тьютора в Study Mode
    ...
    for pattern in patterns:
        ...
        if matches and len(matches) >= 2:  # Минимум 2 варианта
...
for idx, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        ...
        # Если это сообщение с вариантами ответов квиза и оно последнее (для Learn Mode)
        ...
        # Для Study Mode: парсим и показываем кнопки с вариантами ответов, если это последнее сообщение тьютора
        if (st.session_state.mode == "study" and
            message["role"] == "assistant" and
            idx == len(st.session_state.messages) - 1):

            # Проверяем, не сохранены ли уже кнопки (чтобы не парсить повторно)
            if "study_quiz_options" not in message:
                parsed_options = parse_quiz_options(message["content"])
                if parsed_options:
                    message["study_quiz_options"] = parsed_options

            # Если есть опции для отображения
            if "study_quiz_options" in message:
                st.markdown("---")
                st.markdown("**Выбери ответ:**")
                cols = st.columns(2)
                for i, option in enumerate(message["study_quiz_options"]):
                    col_idx = i % 2
                    with cols[col_idx]:
                        # Используем префикс study_ чтобы не конфликтовать с Learn Mode
                        if st.button(f"{option}", key=f"study_quiz_{idx}_{i}", use_container_width=True):
                            st.session_state.study_quiz_answer = option
                            st.rerun()
...
# Обработка ответа из кнопки квиза Study Mode
if "study_quiz_answer" in st.session_state:
    question = st.session_state.study_quiz_answer
    del st.session_state.study_quiz_answer

    # Добавляем ответ пользователя в историю
    st.session_state.messages.append({"role": "user", "content": question})

    # Получаем ответ от тьютора
